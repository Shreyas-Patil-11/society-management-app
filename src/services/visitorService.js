...
/**
 * Get all society visitors (Live Data)
 * Backend Route: GET /api/visitors
 */
export const getSocietyVisitors = async () => {
  try {
    ...
    if (response.success) {
      return {
        success: true,
        data: response.data || [], // Backend returns array of visitors
      };
    }
...
/**
 * Mark visitor exit
 * Backend Route: PUT /api/visitors/exit/:id
 */
export const markVisitorExit = async (visitorId) => {
  try {
    // Constructing URL manually to match backend route: /api/visitors/exit/:id
    // VISITOR_ENDPOINTS.LIST is usually ".../api/visitors"
    const url = `${VISITOR_ENDPOINTS.LIST}/exit/${visitorId}`;
    ...
    return { success: false, message: 'Failed to mark exit' };
  }
};
