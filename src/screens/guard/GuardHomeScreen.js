...
const mapVisitorToLog = (visitor) => {
  ...
  // Logic: 
  // If entry is today => "10:30 AM"
  // If entry was yesterday or before => "Yesterday" or Date
  // (Keeping it simple as just time for "Recent" view)
  ...
  return {
    ...
    type: 'Visitor', // Or map from backend if type column exists
    dest: dest,
    time: timeString,
    status: visitor.exit_time ? 'left' : 'inside', // 'left' maps to grey badge, 'inside' to green/blue
    rawDate: date,
  };
};
...
const GuardHomeScreen = ({ navigation }) => {
  ...
  // Fetch Data (Auto-runs when screen comes into focus)
  useFocusEffect(
    useCallback(() => {
